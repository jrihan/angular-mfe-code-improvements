...
export class CNPJ {
  ...
  static formGroupValidator(
    tipoDocumentoControlName: string,
    numeroDocumentoControlName: string,
    cnpjType = 'CNPJ'
  ): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const tipoDocumento = control.get(tipoDocumentoControlName)?.value;
      const numeroDocumento = control.get(numeroDocumentoControlName)?.value;

      if (!numeroDocumento || tipoDocumento !== cnpjType) {
        return null;
      }

      return this.isValid(String(numeroDocumento)) ? null : { cnpj: true };
    };
  }
